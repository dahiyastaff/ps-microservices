...
public class ShoppingBasketController(IShoppingBasketService basketService, Settings settings) : Controller
{
    ...
    public async Task<IActionResult> Checkout()
    {
        ...
        var total = basketLines.Sum(bl => bl.Price * bl.TicketAmount);
        return View(new BasketCheckoutViewModel { BasketTotal = total });
    }

    [HttpPost]
    public async Task<IActionResult> Checkout(BasketCheckoutViewModel basketCheckout)
    {
        var basketId = Request.Cookies.GetCurrentBasketId(settings);
        await basketService.Checkout(basketId, basketCheckout.BasketTotal, basketCheckout.CardNumber);
    ...
    {
        Response.Cookies.RemoveBasket(settings);
        return View();
    }
}
